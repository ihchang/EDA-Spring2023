...
#paste brings together strings, there is paste and paste0, paste does not insert spaces. the ^ is to the power of. mean is green letter mu
...
#for the gg save we can just put the name, and if you dont have a folder, then it will create one for you
#warning is like you might want to second a look, but error is bad, you need to fix it
```
...
processed_data = "Data/Processed_KEY" #this is a string but it matches the folder structure on our machines
#here, once you import the package, will always point to where the Rproj folder is, its more stable than getwd()
...
#What kind of values are months? We use a lot of categorical data, so we string as factors, and we might have some numerical data that we might want to have as factors. Think of lake names, that means that we can have 350 records, and there are 2 levels, such as peter lake or paul lake. we have in our data set, this column and month, we might think should this be a factor?
...
#we cant use histogram because it doesnt show all of our months
#we have to think about how we are treating our months and we want to have each month to be distinct. We need to convert our month to a factor. So 2 has nothing to do with month 3. We are going to treat our months as factors

#Convert month to a factor -- with 12 levels, labelled with month names
factor(PeterPaul.chem.nutrients$month,
       levels = 1:12,
       labels = month.abb) #has converted all the numeric months to names

#this shows that this has levels, unique values. these are treated as categories so now if we went back 
...
#this crunches our year, we want to keep our empty months and instead of numbers here, add month names. This is where learning visualization is hard. we need to play around with the axis. we want to show the empty categories, so we put drop as false. + are for ggplot commands
...
#to get the degree sign you can do ALT 248
...
#alpha sets the transparency, 0 will either be fully opaque or not
