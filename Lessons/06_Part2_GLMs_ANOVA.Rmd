...
### One-way ANOVA
A one-way ANOVA is used when you have one categorical explanatory variable with more than two categories. In R, we can run the model with the function `lm` or `aov`, the latter of which will allow us to run post-hoc tests to determine pairwise differences. We may need to determine whether three or more populations have equal means. Then we have to conduct a formal hypothesis test. This is an analysis of variance (ANOVA). Lets say we are looking at 4 diff populations. We are looking at n populations at 4 levels. The ANOVA will help you determine if the variance is due to the different locations or different levels. levels (the populations)

For example: Does total litter biomass differ among sites at the Niwot Ridge LTER? we want to check if the mean of the biomass differs among the sites. we will have to wrangle the data first, so that the subset only has the information that we need.
```{r}
# Wrangle the data
...
#results: reject null in all except two: NIWO_057 and NIWO_046, the data for the first one does not follow normal distribution but that doesnt mean we cannot run the anova test
#but method is robust from small deviations from normal distribution
qqnorm(Litter.Totals$dryMass); qqline(Litter.Totals$dryMass) #the data clearly does not follow a normal distribution

# Test for equal variance
# ANOVA is robust against departures from equal variance.
# bartlett.test() performs Bartlett's test of the null that the variances in each of the groups (samples) are the same. will check the null that the variance of each of the levels are the same. 
bartlett.test(Litter.Totals$dryMass ~ Litter.Totals$plotID)

#results: reject null i.e. variances are not equal

# Format ANOVA as aov
Litter.Totals.anova <- aov(data = Litter.Totals, dryMass ~ plotID) #drymass is dependent and plotID is categorical
summary(Litter.Totals.anova)

#results: reject null hypothesis i.e. difference between a pair of group means is statistically significant. The null is the mean is the same across all different sites, so if we are rejecting that then we are saying that the means are not all the same. we know that the means are not all the same, but its not telling us which means are not the same. so we need to run more test to find out which ones are different.

# Format ANOVA as lm; the main difference will be the outputs and the summary table. 
...
# Post-hoc test
# TukeyHSD() computes Tukey Honest Significant Differences is the most common for testing all group variances
TukeyHSD(Litter.Totals.anova)

# Extract groupings for pairwise relationships
Litter.Totals.groups <- HSD.test(Litter.Totals.anova, "plotID", group = TRUE) #group=TRUE means that it will group things that have the same mean. the results are showing you some possible treatment groups. 
...
> the first thing we need to do is to have the same order as our group list here, becaue all the a and b groups would be together and see the changes in the mean.
...
#rejecting the null, the mean across the groups are different. 
...
#the interaction is significant among the variables
