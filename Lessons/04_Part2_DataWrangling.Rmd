...
NTL.nutrient.data <- read.csv("./Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv",stringsAsFactors = TRUE) #we are now pulling in two data sets
```
...
NTL.nutrient.data.PeterPaul <- droplevels(NTL.nutrient.data.PeterPaul) #if you want the other lake names to go away
summary(NTL.nutrient.data.PeterPaul$lakename)
...
```{r}
#gather is you are transforming columns into rows
#spread you are transforming rows into columns, so you are making it wider
...
#you want to gather the nutrient data into one column. you need to add two columns, transform 4 columns into two
#but now there is a lot of NAs, so if there is a row that has NA, you should remove the rows that have NA
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration)) #use the function subset, if you have an NA for the row correlated to concentration
count(NTL.nutrient.data.PeterPaul.gathered, nutrient) #for each nutrient, how many observations do we have?
...
#store the processed data into another file
...
Litter <- LitterFiles %>%
  plyr::ldply(read.csv) #you are going to use a pipe, for Litter files
```
...
LitterTrap <- left_join(Litter, Trap, by = c("siteID", "plotID", "trapID")) #you are mixing the 23 variables from trap minus the three that they have in common
