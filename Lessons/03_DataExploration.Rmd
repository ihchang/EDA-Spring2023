...
> Answer: it shows you all the packages that are associated with it, in the case of tidyverse
...
str(USGS.flow.data) #show your data frame and all the objects
dim(USGS.flow.data) #how many rows and how many columns
length(USGS.flow.data) #how many columns

head(USGS.flow.data) #shows the first few rows (no specification, shows 6)
...
> Answer: For many instances we have NAs because there was no data imported. 
...
```{r Removing NAs}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #creates a new data file without the NAs, the new data frame will show up in the environment
dim(USGS.flow.data)
