...
vector1[3] # everything in a vector needs the same data type, one dimension just keep adding rows of values. this should return 5.3

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # specify its dimensions in terms of rows and columns
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #enter these by row because the default is columns
matrix2
...
matrix1[4, ] #this gives you the 4th row, but you don't specify columns so it will give you all of them
matrix1[ , 3] #
matrix1[c(12, 14)] #
matrix1[c(12:14)] #
matrix1[2:4, 1:3] #
...
matrix4
#a matrix is just an array or something like that. an array is the same location over time, so you have a stack by 2x2, and then there can be multidimen arrays. all the values in these things must be the same data type and allow you to do really fast calculations.

# Lists ---- #you can throw anything in a list, so this list will have 4 items in it
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1 #each of the items we give a name so we can retrieve it. the ; allows us to put 2 codes on one line
list1[[2]] #you can refer to items within the index, but it needs double brackets
#to rename: list1['name'] <- "mario"
...
#a data frame is like a list of vectors
...
#how do we get a vector from our data frame? use the $ so that you can find the sd
```
...
today <- Sys.Date() #utc: universal time coordinate
