---
title: "IreneChang_A07_CraftingReports.Rmd"
author: "Irene Chang"
date: "Spring 2023"
output:
...
```{r}
library(knitr)
EPA.Air.Quality.Data <- matrix(c(
  "Source", "Date", "Filename", "EPA Air Quality System (AQS)", "2018-2019", "EPAair_O3_PM25_NC1819_Processed.csv"), ncol = 2, byrow = FALSE)
kable(EPA.Air.Quality.Data, align = "rl")
```
...
```{r plot.Ozonelevels2018, echo=FALSE, fig.align='left', fig.cap='Ozone Levels By Month', fig.height=2.5, warning=FALSE, message=FALSE}
EPA.Ozone.2018 <- epa_data %>% 
  filter(Year == 2018) %>% 
  na.omit(Ozone)

EPA.Ozone.2018.boxplot <-
  ggplot(EPA.Ozone.2018, aes(x= factor(Month), y= Ozone)) +
  geom_boxplot() +
  labs(
...
print(EPA.Ozone.2018.boxplot)
```

```{r plot.Ozonelevels2019, echo=FALSE, fig.align='left', fig.cap='Ozone Levels By Month', fig.height=2.5, warning=FALSE, message=FALSE}
EPA.Ozone.2019 <- epa_data %>% 
  filter(Year == 2019) %>% 
  na.omit(Ozone)

EPA.Ozone.2019.boxplot <-
  ggplot(EPA.Ozone.2019, aes(x= factor(Month), y= Ozone)) +
  geom_boxplot() +
  labs(
...
You can find a link to my GitHub repository 
[here](https://github.com/ihchang/EDA-Spring2023)
